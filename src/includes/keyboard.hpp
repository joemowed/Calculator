...
#include <array>
class Keyboard {
    const uint16_t DEBOUNCE_COUNT = 3; // The number of times the IRQ can read the key as high before the key will be registered as pressed again
  public:
    const struct keyCodes {
        ...
        const char BUFFER_EMPTY = 0x7; // character code to return when the key buffer is empty
    } keyCodes;
  ...
  private:
    // shifts all elements in buffer down one position
    void reduceBuffer();
    // writes new element to buffer
    void pushToBuffer(char keyCode);
    // sets BUFFER_LENGTH to 0
    void clearBuffer();
    ...
    const std::array<const Key, 21> keys = {{
